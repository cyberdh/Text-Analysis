  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput(inputId = "word_choice",
                  label = "Choose a word:",
                  choices = c("loue","selfe","shee","right", "ballance","waigh"),
                  selected = "loue")
...
server <- function(input, output) {
  observe({
   # setwd("~/Desktop/R/Text_Analysis/shiny/TermFreq2/") #Set directory path for your environment 
    library(ggplot2)
  ...
  output$plot <- renderPlot({
    ...
    title <- paste("Use of '", input$word_choice, "' in Urania and The Faerie Queene")
    mids <- barplot(word.dispersion.df$num,
                    ylab = "Frequency",
                    main = (title), xaxt="n", col = c("darkseagreen1","lemonchiffon"))
    axis(1, at=mids, rownames(word.dispersion.df), srt=225, tick=FALSE, xpd = TRUE, las=2)
